...
const CreateCategoryModal = ({ isOpen, onClose, handleEdit, categoryId, currentPage, totalPages, totalCount, onPageChange, articles, addArticalId, articelIds }) => {
    const [newName, setNewName] = useState('');
    const [newArticelIds, setNewArticelIds] = useState([]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const response = await axios.get(`http://localhost:8000/api/admin/category/${categoryId}`,{
                    headers: {
                        Authorization: `Bearer ${token}`
                    }
                });
                setNewName(response.data.category.name);
                setNewArticelIds(response.data.category.articel_ids)
                setIsLoading(false);
            } catch (error) {
    ...
    const handleSubmit = async () => {
        try {
            ...
            const response =  await handleEdit( categoryId,newName, newArticelIds );
            if(response === 1){
                onClose();
            }
    ...
    const handleCheckboxChange = (id) => {
        if (newArticelIds.includes(id)) {
            setNewArticelIds((prevIds) => prevIds.filter((artId) => artId !== id));

        } else {
            setNewArticelIds((prevIds) => [...prevIds, id]);
        }
    };
                            ...
                            <div key={article.id} className="bg-white p-4 rounded-md shadow-md h-[230px] relative mb-4">
                                ...
                                <div className="flex items-center justify-center mt-2">
                                    <input
                                        ...
                                        checked={newArticelIds.includes(article.id)}
